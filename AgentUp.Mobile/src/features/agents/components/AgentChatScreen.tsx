...
import { decideAgentPermission, getAgent, scheduleAgent, sendAgentMessage, stopAgent, streamAgentEvents } from '../providers/AgentApiProvider';
...
export function AgentChatScreen({ workspace }: { workspace: Workspace }) {
  ...
  const applyEvent = useCallback((event: AgentEvent) => {
    ...
    if (event.type === 'user_message') { setItems(current => [...current, { id: `${event.sequence}`, role: 'user', text: textFrom(event.payload) }]); return; }
    if (event.type === 'session_update') {
      const update = (event.payload as { update?: unknown }).update ?? event.payload;
      const kind = stringField(update, 'sessionUpdate') ?? stringField(update, 'type') ?? 'agent';
      const role = kind.includes('thought') ? 'thought' : kind.includes('tool') ? 'tool' : 'agent';
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      while (!controller.signal.aborted) {
        try { await streamAgentEvents(server, workspace.id, lastSequence.current, applyEvent, controller.signal); }
    ...
    <View style={styles.content}>{tab === 'changes'
      ...
      : <View style={styles.chat}>
          <View style={styles.heading}><View><Text style={styles.chatTitle}>{workspace.displayName}</Text><Text style={styles.status}>{session?.agent ?? 'No agent'} · {session?.state ?? 'loading'}</Text></View>
            {session?.agent && <Pressable accessibilityRole="button" style={styles.stop} onPress={() => server && void stopAgent(server, workspace.id).then(() => setSession(null))}><Text style={styles.stopText}>Stop</Text></Pressable>}
          </View>
...
function textFrom(value: unknown): string { if (typeof value === 'string') return value; if (!value || typeof value !== 'object') return ''; const record = value as Record<string, unknown>; return typeof record.text === 'string' ? record.text : textFrom(record.content ?? record.message ?? record.title); }
function stringField(value: unknown, key: string) { return value && typeof value === 'object' && typeof (value as Record<string, unknown>)[key] === 'string' ? (value as Record<string, string>)[key] : null; }
