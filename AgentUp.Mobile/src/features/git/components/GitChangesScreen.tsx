...
export function GitChangesScreen() {
  ...
  return <SafeAreaView style={styles.screen}>
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <Text accessibilityRole="header" style={styles.title}>Git</Text>
      <Text style={styles.subtitle}>
        {selectedWorkspace ? `${selectedWorkspace.displayName} · ${tree?.branch ?? selectedWorkspace.branch}` : 'Select a workspace first.'}
      </Text>
      ...
      <View style={styles.tree}>
        {nodes.map(node => {
          ...
          return <View key={node.key} style={[styles.row, { paddingLeft: 4 + node.depth * 16 }]}>
...
const styles = StyleSheet.create({
  ...
  content: { padding: 20, paddingTop: 78, paddingBottom: 32, gap: 12 },
