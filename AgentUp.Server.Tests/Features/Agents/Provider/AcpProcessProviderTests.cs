...
public sealed class AcpProcessProviderTests
{
    ...
    public async Task CallAsync_writesJsonRpcAndMatchesResponseById()
    {
        ...
        await provider.StartAsync("codex", Path.GetTempPath(), new Dictionary<string, string>(), CancellationToken.None);
    ...
    public void StartAsync_failsWhenTheExecutableIsMissing()
    {
        ...
        var exception = Assert.ThrowsAsync<InvalidOperationException>(
            async () => await provider.StartAsync("codex", Path.GetTempPath(), new Dictionary<string, string>(), CancellationToken.None));
    ...
    public async Task CallAsync_surfacesJsonRpcErrorsAndMissingResults()
    {
        ...
        await errors.StartAsync("codex", Path.GetTempPath(), new Dictionary<string, string>(), CancellationToken.None);
        ...
        await using var missing = Create(
            UnixCommand(),
            UnixArguments("read line; printf '%s\\n' '{\"jsonrpc\":\"2.0\",\"id\":1}'"));
        await missing.StartAsync("codex", Path.GetTempPath(), new Dictionary<string, string>(), CancellationToken.None);
