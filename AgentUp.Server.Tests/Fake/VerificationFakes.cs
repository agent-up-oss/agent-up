...
/// <summary>Static verification rules chosen by the test.</summary>
...
/// <summary>A configuration source that fails the way a malformed agent-up.json does.</summary>
internal sealed class ThrowingVerificationConfigurationLoader(string message)
    : IVerificationConfigurationLoader
{
...
internal sealed class StaticChangedContentSource(IReadOnlyDictionary<string, string> changed)
    : IChangedContentSource
{
...
internal sealed class ScriptedCheckRunner(IReadOnlyDictionary<string, int>? exitCodes = null) : ICheckRunner
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        return Task.FromResult(new CheckOutcome(check.Id, check.Command, exitCode, 7, "captured output"));
    }
}
