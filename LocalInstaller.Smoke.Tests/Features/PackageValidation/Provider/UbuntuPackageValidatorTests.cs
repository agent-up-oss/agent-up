...
public class UbuntuPackageValidatorTests
{
    ...
    public async Task ValidateAsync_reportsExpectedPathsAndContractFindings()
    {
        ...
        try
        {
            var result = await new UbuntuPackageValidator(new UbuntuPackageArchiveProvider(commands)).ValidateAsync(new PackageValidationRequest("ubuntu", "linux-x64", artifactDir, workDir, AgentUpProduct()));
