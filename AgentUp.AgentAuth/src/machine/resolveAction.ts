 ...
 */
export function resolveAction(challenge: AgentLoginChallenge | null | undefined): AgentLoginAction {
  const waiting = { kind: 'wait', message: 'Waiting for the agent to print a sign-in link.' } as const;
  if (!challenge || !challenge.url) return waiting;
  ...
  if (transport === 'redirect') {
    ...
    if (!challenge.redirectUri) return { kind: 'open', url, message };
  ...
  if (transport === 'code') {
    ...
    if (challenge.code) return { kind: 'openWithCode', url, code: challenge.code, message };
...
export function normalizeTransport(transport: string | null | undefined): AgentLoginTransport {
  const value = (transport ?? '').toLowerCase();
  if (value === 'poll' || value === 'code' || value === 'redirect') return value;
  return 'unknown';
}

/** True once a challenge has expired, so a client can offer to start again rather than hang. */
export function hasExpired(challenge: AgentLoginChallenge | null | undefined, now: Date = new Date()): boolean {
