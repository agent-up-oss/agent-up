...
Legacy local application commands and legacy Docker `services` remain supported. Local application commands are executable-plus-arguments strings, not shell expressions; the Server launches them directly with an argument list and rejects shell chaining, redirects, variable expansion, and subshells. Named root arrays whose names match enabled runtime-kind module ids (including first-party `dotnet` and `docker`) bind through `IRuntimeCapability`; unknown extra keys and missing required extra keys fail. Package version is the module contract; technology version (`sdk`) is an input to `Deliver`. The Server exposes capability status to Desktop, CLI, and automation clients.
