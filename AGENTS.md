...
| `AgentUp.Sdk.Common` | Shared kind, identity, Nix declaration, and registration types for capability SDKs |
| `AgentUp.Sdk.Runtime` | MSBuild SDK and `IRuntimeCapability` contract for runtime-kind modules |
| `AgentUp.Sdk.Agent` | MSBuild SDK and `IAgentCapability` contract for agent-kind modules |
| `AgentUp.Capabilities.Abstractions` | Generic `capability.json` schema, launch plans, validation records, and (legacy, unused) adapter interfaces |
| `AgentUp.Capabilities.Common` | Generic Nix invocation, index merge, and CLI executable discovery used by the Server runtime |
| `AgentUp.Capabilities.Dotnet` | First-party Runtime SDK consumer that packs the `dotnet` registry package |
| `AgentUp.Capabilities.Docker` | First-party Runtime SDK consumer that packs the `docker` registry package |
| `AgentUp.Capabilities.Codex` | First-party Agent SDK consumer that packs the `codex` registry package |
| `AgentUp.Capabilities.Cursor` | First-party Agent SDK consumer that packs the `cursor` registry package |
| `AgentUp.Capabilities.Claude` | First-party Agent SDK consumer that packs the `claude` registry package |
| `AgentUp.Registry` | Local-directory store and remote HTTP catalog for versioned capability packages. Packages and push staging live under the configured registry root, resolved when the store is first used so nothing is written beside the running project. Not workspace orchestration; Server remains source of truth for enablement and launches |
...
`agent-up start` posts workspace identity plus legacy `applications`, `desktopApplications`, and `services`, and every other `agent-up.json` root array as `runtimeSections`. It does not bind those arrays; the Server matches names to enabled runtime-kind module ids. User guide: `docs/user-docs/workspaces/index.md`.
...
Legacy local application commands and legacy Docker `services` remain supported. Local application commands are executable-plus-arguments strings, not shell expressions; the Server launches them directly with an argument list and rejects shell chaining, redirects, variable expansion, and subshells. WrapLaunch sends an executable through an enabled runtime module's Nix environment only when that module `provides` it, so enabling `dotnet` and `docker` does not wrap Docs or Mobile `npm` through those shells. Packed `default.nix` for a `dotnet-sdk*` module sets `DOTNET_ROOT` to that Nix SDK and unsets testhost `MSBuildSDKsPath` so `dotnet run` writes `runtimeconfig.json` and the apphost finds `libhostpolicy.so` under `shared/Microsoft.NETCore.App`. It also delivers `icu` and `openssl` and puts both on `LD_LIBRARY_PATH`: the runtime opens them with `dlopen` by soname rather than through a linked RPATH, so nixpkgs cannot patch them in, and an application missing either aborts at startup - globalization pointing at `aka.ms/dotnet-missing-libicu`, TLS with `No usable version of libssl was found`. The shell is `mkShellNoCC`: a capability shell runs tools rather than compiling C, and `mkShell` pulled the whole stdenv into every host that enabled a module. Named root arrays whose names match enabled runtime-kind module ids (including first-party `dotnet` and `docker`) bind through `IRuntimeCapability`; unknown extra keys and missing required extra keys fail. Package version is the module contract; technology version (`sdk`) is an input to `Deliver`. The Server exposes capability status to Desktop, CLI, and automation clients.
...
| `AgentUp.Sdk.Common` | `AgentUp.Sdk.Common.Tests` |
| `AgentUp.Sdk.Runtime` | `AgentUp.Sdk.Runtime.Tests` |
| `AgentUp.Sdk.Agent` | `AgentUp.Sdk.Agent.Tests` |
...
| `AgentUp.Registry` | `AgentUp.Registry.Tests` |
...
`AgentUp.Tests` is a separate cross-product E2E project that exercises the full Desktop application and shared Installer application through platform fixture adapters. Desktop browser behavior that only exists once a real WebView engine and a real platform storage provider are involved — the WebView upload bridge and OAuth sign-in through redirects and native popups — belongs here rather than in headless tests, and must substitute only the modal dialogs and native engine callbacks a CI runner cannot drive. Linux uses `AgentUp.Fixtures.Linux` with Xvfb and WebKitGTK: the adapter always starts a private Xvfb, `XDG_RUNTIME_DIR`, and session bus, imports `PATH` and native libraries from `nix-shell shell.nix` on a NixOS host so IDEs do not need extra env vars (`AGENTUP_E2E_IMPORT_NIX_SHELL` overrides the host check; anywhere else the system GTK/WebKit are used, because mapping the Nix closure beside the libraries Avalonia already loaded segfaults the test host), and Avalonia is forced onto X11, so a developer workstation's Wayland/X11 session is not reused. Set `AGENTUP_E2E_USE_SESSION_DISPLAY=1` only to debug against the real display. macOS uses `AgentUp.Fixtures.MacOs` and isolates test home and temporary storage. Windows uses `AgentUp.Fixtures.Windows` and isolates WebView2 profile storage through `WEBVIEW2_USER_DATA_FOLDER` only: it must leave `LOCALAPPDATA` and `APPDATA` pointing at the real profile, because redirecting them isolates nothing Desktop stores -- `Environment.GetFolderPath` asks the shell rather than the environment on Windows -- while the WebView2 browser process does inherit them and never finishes starting from an empty profile root, which hangs the UI thread before any test runs. Isolating Desktop's own storage on Windows needs a storage-root override in the product, which does not exist yet. Platform-specific canary tests enforce both isolation contracts; successful native Avalonia/WebView startup proves the hosted desktop session itself. Both start Avalonia against the native desktop/WebView backend available on the CI runner. These tests are part of the normal platform test run: Ubuntu runs them in the GUI test job, while the macOS and Windows package jobs execute a framework-dependent test artifact produced by the Ubuntu payload job against the matching .NET runtime those jobs install. macOS CI runs the project through its NUnitLite executable entry point so Avalonia Native initializes on the process main thread while still exercising the same test fixtures and native WebView.

Runtime capability E2E (`Category=RuntimeCapabilityE2E`) starts the repository-root `agent-up.json` from Desktop after enabling the `docker` and `dotnet` modules, then replays the recorded Example Web validation flow. That is the proof those capabilities hosted Postgres and the .NET API in the same workspace as Docs, Example Web, and Sample Desktop. CI runs it in the Runtime capability E2E job. That job gates the release, not the native `platform` matrix: it installs a .NET SDK closure, npm trees, Chromium and Postgres before its first assertion, and blocking every native test behind it delayed work it does not verify. The job packs first-party capability packages and sets `AGENTUP_SKIP_DEV_AGENT_BOOTSTRAP=1` so `shell.nix` does not download Cursor, Codex, or Claude CLIs; those stay capability-managed. It is excluded from the 180-second E2E watchdog, the default GUI `AgentUp.Tests` filter, and native macOS/Windows `AgentUp.Tests` runs. Almost all of its wall-clock time is spent in the fixture before the first assertion, so the job runs it at detailed console verbosity and with `--blame-crash`: the fixture prints each stage it reaches, and a test host that dies leaves both that trail and a dump in the uploaded results. A stage line goes to `TestContext.Out` as well as `TestContext.Progress`, because Progress alone never reaches the job log at default verbosity. Every wait that can time out fails with the five application consoles attached, and stops early once the Server reports an application `Failed` - a port poll alone spends its whole deadline on a process that aborted in the first second and still names neither the application nor its error.
...
The disposable native sign-in harness permits cleartext traffic only so its Android emulator and iOS simulator can reach ephemeral Server and identity-provider processes on the CI host. That plugin sets `usesCleartextTraffic` and writes a network-security-config that allows `10.0.2.2` and loopback; a generated config that forbids cleartext wins over the manifest flag on API 28+. Production Mobile transport policy must not inherit that exception.
...
Native Detox launches the harness with `agent-up-chat://connect` so the Server origin and workspace arrive without typing into the connect form. Android Fabric's `replaceText` does not fire `onChangeText`, which left Connect as a no-op and the suite waiting for a picker the chat never mounted. The installable-web suite still fills the form because Playwright's fill does.
...
- Provider tests verify low-level external behavior in isolation, including filesystem providers, command/tool providers, environment providers, platform adapters, package writers/stagers, probes, generated directory state, and process-style command shapes. Temp directories are allowed when the provider boundary requires them. Packed capability DLLs are the launch contract. When a DLL is missing, Server may fall back to the packed `Launch` template so enablement still works. These tests must not skip based on whether a CLI is present.
...
Each workspace has one active ACP agent session and a separate saved session history. Desktop chrome for the live session is the **Agent** tab; Mobile chrome for the picker is the **Agents** tab. Subscription sign-in is Server-owned. Public agent identity is the enabled module id; Server ACP session code stays generic.
...
The Agent-Up main release workflow publishes `@agent-up/audit` to npm with the planned release version when `NPM_TOKEN` is configured. The same release also publishes the Server container and `agent-up-helm` chart to Docker Hub (`themassiveone/agent-up-server` and `themassiveone/agent-up-helm`) when `DOCKERHUB_USERNAME` and `DOCKERHUB_TOKEN` are configured. Android and iOS store binaries ship from `.github/workflows/mobile-ci.yaml` on `workflow_dispatch`; see `docs/developer-guide/repo/mobile-store-release.md`. Path-filtered pushes of that workflow only smoke-build and sign. Both push and dispatch upload the signed AAB and IPA to GitHub Actions artifacts with 1-day retention before any store submit. Helm `capabilities.enabled` seeds Server `enabled.json` with package id/version pairs. The Server image installs Nix (non-root UID 1654) with a persistent store on the Server volume, plus `git` and first-party packer inputs. Native Windows Server is not a first-class Nix host; run Server on WSL2/Linux. There is no second non-Nix capability install path.
...
Packaging from NixOS or other non-native hosts should use the wrapper scripts in `scripts/package-*.sh`, which enter target-specific shells from `packaging/nix/` before delegating to the packaging entrypoint. NixOS installs Agent-Up declaratively through generated NixOS/Home Manager module options; `AgentUp.InstallerApp` is still shipped as a lookup-only dashboard through `agent-up-installer`, with install/update/uninstall actions disabled. Desired capabilities are declared through `services.agent-up.capabilities` or `programs.agent-up.capabilities` and written to `/etc/agent-up/enabled.json` (or the Home Manager equivalent). Runtime enablement reads `AGENTUP_CAPABILITY_ENABLED_PATH` as a seed into the Server data directory; launches wrap through Nix using packages in `AGENTUP_CAPABILITY_REGISTRY_PATH`. The repository launch profile sets `AGENTUP_CAPABILITY_REGISTRY_PATH` and `AGENTUP_CAPABILITY_ENABLED_PATH` to `.agent-up-dev/capability-registry` and `.agent-up-dev/enabled.json`. When those paths are unset, a development Server also uses `.agent-up-dev/capability-registry` if it contains a packed `index.json`. Desktop and Mobile enable modules through Server APIs and never call the remote registry. Agent-Up does not ship installer zip-modules or a second non-Nix capability install path. Native Windows Server hosts must use WSL2/Linux. `nix-shell shell.nix` seeds a local enabled set under `.agent-up-dev` for workstation testing. Installed-service smoke validates the native package and the service it installs, not capability launches: CI sets `AGENTUP_CAPABILITY_SMOKE_SKIP_REAL=1` for it, because a package runner provisions no Nix, no packed registry, and no enabled set, and real launches need all three. Real capability launches are proven by the Runtime capability E2E job. With that flag unset, the smoke launches one .NET app and one Docker app through `agent-up.json` `dotnet[]`/`docker[]` shims and validates individual app stop/start plus workspace stop; the generated .NET smoke app is restored and built before `agent-up start` so lifecycle validation does not depend on first-run SDK restore/build timing. The Docker sample uses `nginx:alpine` on Linux and macOS and a matching Windows IIS image on Windows runners, with `AGENTUP_CAPABILITY_SMOKE_DOCKER_IMAGE` available for CI pre-pull/override. macOS packaging still requires Darwin because Apple package, signing, and notarization tools are not available on Linux.
