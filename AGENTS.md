...
Each workspace has one ACP agent session. Desktop chrome for the live session is the **Agent** tab; Mobile chrome for the picker is the **Agents** tab. Subscription sign-in is Server-owned. Public agent identity is the enabled module id; Server ACP session code stays generic.
