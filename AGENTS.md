...
Mobile route entrypoints stay thin under `src/app/`; product UI and client behavior live in capability-oriented slices under `src/features/`. Do not commit Expo-generated `android/` or `ios/` projects unless native customization is intentionally adopted. Store AABs and IPAs are produced by the manual `.github/workflows/deploy-mobile.yaml` workflow with CI `expo prebuild` and Fastlane Match; they are not part of the `ci.yml` desktop release. The mobile client displays Server-owned state and must not own orchestration.
