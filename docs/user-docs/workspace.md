...
Git worktrees are the recommended model for AI agents working in the same repository because they preserve separate branches, running applications, browser sessions, and validation state. The Desktop Overview tab has a branch dropdown for local and remote-tracking branches, Fetch/Pull/Push, plus workspace identity and Server-owned CPU, memory, storage, and process totals. On Mobile those Git controls live on the workspace Git tab, beside Apps and Agents.
...
Agent-Up can also manage the clone itself instead of only registering a checkout you created. Both the Desktop workspace list and the Mobile sidebar workspace list have a `+` button that asks for a repository and a branch. Agent-Up clones that repository at that branch into its source clones directory and registers the resulting workspace, so it appears in the workspace list on every connected client. See [Mobile](./mobile.md).
