...
CI proves the first-party `docker` and `dotnet` capabilities by starting the repository-root `agent-up.json` from Desktop and replaying the recorded Example Web flow. That check lives in `AgentUp.Tests` under `Category=RuntimeCapabilityE2E` and runs in the Runtime capability E2E job rather than the 180-second GUI E2E watchdog.
