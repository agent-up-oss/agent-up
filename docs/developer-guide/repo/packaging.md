...
`LocalInstaller.Core` owns installer contracts and deterministic planning:
...
`LocalInstaller.App` owns the shared installer dashboard, while `AgentUp.InstallerApp` only supplies Agent-Up product configuration:
...
- Installed capability-module grid is Server-owned. Agent-Up does not ship installer zip-modules; Desktop and Mobile enable registry packages through Server APIs.
- Official capability packages are packed into the local registry in CI (`scripts/pack-first-party-capabilities.sh`) before tests run. Optional release jobs may push to the remote Registry feed.
- CI verifies `LocalInstaller.App` and the thin `AgentUp.InstallerApp` entrypoint build without `AgentUp.Capabilities.*` project references; the app owns installer-facing catalog contracts and can render the module section with either the Agent-Up catalog or the null catalog implementation.
...
- Noninteractive installer operations through `AgentUp.InstallerApp --smoke-installer-operations --payload-root <payload-root>` for smoke tests and CI workflows that must exercise the shipped installer executable without driving pointer clicks. This smoke command installs, repairs, updates, and uninstalls `desktop`, `server`, and `cli` individually before running the bundled core install and validation; `server` includes tray payload and autostart registration. Focused commands are also available for `--install-core`, `--validate-installed`, and `--install-component`, `--update-component`, `--repair-component`, or `--uninstall-component` with a component target.

Ubuntu, macOS, and Windows have real installer adapters behind the guided app. The app uses the real adapter by default and requires a payload root containing `desktop`, `server`, `cli`, and `tray` directories; Ubuntu and macOS payload roots also carry `icon/<ProductName>.png` for launcher and app bundle registration. The payload root may come from `AGENTUP_INSTALLER_PAYLOAD_ROOT`, from a bundled offline payload in the installer app, or from a future online payload download. Tests and local non-privileged flow checks opt into the fake adapter with `AGENTUP_INSTALLER_FAKE=1`.

On Ubuntu, the default adapter installs the staged Desktop, Server, CLI, and tray payload into `/opt/agent-up`, registers `agent-up-server.service`, creates `/usr/bin/agent-up`, writes the desktop launcher with a `StartupWMClass`, registers `/etc/xdg/autostart/agent-up-tray.desktop`, and validates the installed state through systemd, tray autostart lookup, and a fresh-shell CLI lookup. Tray payload and login autostart are installed when the Server target is selected. `AgentUpServerManifest` may declare service environment variables through LocalInstaller `1.1.0+`; Ubuntu writes them to `agent-up-server.service.d/10-environment.conf`, Windows writes the service registry `Environment` value, and macOS extends the launchd plist `EnvironmentVariables` dictionary. Installed-service smoke sets `AGENTUP_PACKAGE_SMOKE_AUTH_DISABLED=true` so install-core can inject `AGENTUP_AUTH_DISABLED=true` for validation only; production packages do not ship that variable by default.
...
On Windows, the default adapter installs the staged Desktop, Server, CLI, and tray payload under `Program Files\Agent-Up`, removes any previous `agent-up-server` Windows Service before registering the new one with restart policy, writes the CLI shim, adds the installer-managed `bin` directory to machine `PATH` without duplicating it, creates the Start Menu shortcut, registers machine-level tray login auto-start, registers uninstall metadata for Apps & Features, and validates the installed state through `sc.exe`, tray auto-start lookup, and a fresh-shell CLI lookup. Tray payload and login auto-start are installed when the Server target is selected.

`LocalInstaller.Packaging` owns release artifact build orchestration, while `AgentUp.Packaging` only supplies Agent-Up product configuration:
...
CI uses prebuilt payload mode: the Ubuntu .NET payload job restores once per release runtime, then publishes single-file self-contained InstallerApp, Desktop, Server, CLI, thin `AgentUp.Packaging`, and thin `AgentUp.PackageSmoke` artifacts for that runtime with restore disabled. CI also builds the JetBrains plugin from `Plugins/Jetbrains` with the planned release version injected through Gradle and publishes `agent-up-jetbrains-plugin.zip` as a GitHub release asset. When `JETBRAINS_MARKETPLACE_TOKEN` is configured, the release job also runs Gradle `publishPlugin` after the GitHub release succeeds.

The Ubuntu .NET payload job uploads one short-lived GitHub Actions artifact per platform job, and native package jobs download only their own runtime slice before passing `--payload-root` to package those exact payloads. Native package jobs wait on the payload, test, GUI test, and coverage jobs plus version. Native package jobs should delete the consumed CI-transfer artifact after download. Native package jobs should not restore or build product .NET projects. They execute the prebuilt native-display E2E runner, then native packaging tools and package/installer smoke validation.
...
The same release job publishes `docker.io/themassiveone/agent-up-server` and the `agent-up-helm` OCI chart to `oci://registry-1.docker.io/themassiveone`. Required Docker Hub secrets are `DOCKERHUB_USERNAME` and `DOCKERHUB_TOKEN`. The chart deploys only the Server; Desktop, CLI, and installer payloads stay on the native-package path. Cluster GitOps consumes that chart from Docker Hub rather than Harbor. Helm `capabilities.enabled` seeds `/etc/agent-up/enabled.json` with package id/version pairs and sets `AGENTUP_CAPABILITY_ENABLED_PATH`. The Server image installs Nix as UID 1654 with `/nix` persisted on the Server volume, plus `git`. Native Windows Server is WSL2/Linux; there is no second non-Nix capability path.

`LocalInstaller.Smoke` owns smoke validation, while `AgentUp.PackageSmoke` only supplies Agent-Up product configuration:
...
- Native package install, InstallerApp-driven core installation, service readiness, installed CLI workspace registration, diagnostics, and uninstall cleanup for installed-service smoke tests.
...
The generated Nix package-set flake exports both `nixosModules.default` and `homeManagerModules.default`. The NixOS module registers the Server as the system `agent-up-server.service`. The Home Manager module installs Desktop, CLI, and the lookup-only InstallerApp dashboard and, by default, registers a user `agent-up-server.service` so Home Manager-only installs still provide a local Server on port 5000; users can disable that user service when a system service already owns the Server. Generated Nix service units run from the packaged Server directory, set the ASP.NET content root to that directory, and set a stable .NET bundle extraction directory so runtime file watchers stay scoped to Agent-Up-owned paths. Desired capabilities are declared through `services.agent-up.capabilities` or `programs.agent-up.capabilities` and written to `enabled.json`. Runtime enablement seeds from `AGENTUP_CAPABILITY_ENABLED_PATH` into the Server data directory; launches wrap through Nix using packages in `AGENTUP_CAPABILITY_REGISTRY_PATH`. The `agent-up-installer` launcher sets `AGENTUP_INSTALLER_NIXOS_LOOKUP_ONLY=1`; the dashboard can inspect component state, but install/update/uninstall actions stay disabled because Nix owns system mutation. Enable or disable capability packages through Server APIs or the Nix module seed.

Windows packaging uses WiX through `LocalInstaller.Packaging`. The packaging app generates `Product.wxs`, `Bundle.wxs`, the CLI shim, and the bootstrapper license file, stages required WiX extension DLLs on Windows, then invokes `wix build` for the staged `Product.msi` and bootstrapper executable.
...
macOS packaging uses `Product.pkg` through `LocalInstaller.Packaging`. The package installs only the dashboard `Agent-Up Installer.app`; Desktop, Server, CLI, launchd, symlink, validation, and uninstall behavior stay owned by the InstallerApp and its macOS adapter. The packaging app stages the InstallerApp bundle, its bundle icon resource, its bundled offline `desktop`, `server`, `cli`, and icon payload, installer-app package scripts, distribution XML, and `pkgbuild`/`productbuild` command shapes while consuming macOS install metadata, plist generation, and package scripts from `LocalInstaller.Core`.
...
When any native packaging tool is invoked from `LocalInstaller.Packaging`, tests should assert the exact command shape with an isolated fake command runner and smoke tests should verify the produced artifact on an appropriate runner.

`AgentUp.Packaging package <platform> <runtime-id> <version> [output-dir] --payload-root <path>` packages an existing payload root containing `installer`, `desktop`, `server`, `cli`, and `tray` directories. Without `--payload-root`, the command keeps the local developer fallback of publishing those projects before packaging.

Package smoke scripts must use `AgentUp.PackageSmoke validate-package` for macOS, Windows, and Ubuntu artifact contract checks. macOS package smoke validates the InstallerApp-only `.pkg` and its bundled offline payload. Windows package smoke validates both the Burn bootstrapper and the named MSI sidecar artifact, then runs the bootstrapper `/layout` command to prove the executable package can service layout requests without depending on Burn extracting embedded payloads to a specific directory. Installed-service smoke scripts must use `AgentUp.PackageSmoke validate-installed-service`; the validator installs the native package, runs the installed InstallerApp with `--payload-root <installed-payload> --install-core`, then validates service readiness, installed CLI behavior, diagnostics, and uninstall cleanup. The PackageSmoke CLI accepts `--product-manifest <path>` for package, installed-service, and installer-flow smoke so a second product can run the complete smoke suite without recompilation. Product metadata used by installed-service smoke must be validated before it becomes package names, service names, CLI shims, filesystem path components, shell snippets, or environment data. Windows installed-service smoke validates native Apps & Features registration by DisplayName because MSI and Burn own the uninstall registry key names. Guided installer smoke launches the packaged `AgentUp.InstallerApp --smoke-installer-operations --payload-root <payload-root>` when `AGENTUP_INSTALLER_APP_COMMAND` is set, then runs `AgentUp.PackageSmoke validate-installer-flow <platform> <work-dir> [payload-root]` for the shared workflow contract. Passing a payload root lets the real platform adapter perform the installer work, while `AGENTUP_INSTALLER_FAKE=1` keeps tests non-privileged. CI uses fake mode for the guided installer flow because it validates the shipped executable and shared flow without taking machine-level install ownership. Shell smoke code should stay limited to argument forwarding and runner setup.

Windows installed-service smoke installs and uninstalls the native package through the MSI sidecar with `msiexec`, runs the installed InstallerApp core install command, starts the service, and then validates readiness; the Windows `.exe` remains the user-facing bootstrapper path. Installed-service smoke can also launch one .NET app and one Docker app through capability declarations, verify they are registered with capability status, stop and restart the .NET app independently, stop the Docker app independently, and then stop the workspace. CI does not run that part: the package jobs set `AGENTUP_CAPABILITY_SMOKE_SKIP_REAL=1`, because a package runner has no Nix, no packed capability registry, and no enabled set seeded into the installed service, and a capability launch needs all three. The `Runtime capability E2E` job provisions those and is where real launches are proven. The generated .NET smoke app is restored and built before `agent-up start` so lifecycle validation does not depend on first-run SDK restore/build timing. The Docker capability sample uses `nginx:alpine` on Linux and macOS and a matching `mcr.microsoft.com/windows/servercore/iis` image on Windows runners. Windows CI pre-pulls that image and passes it through `AGENTUP_CAPABILITY_SMOKE_DOCKER_IMAGE` so image download time does not consume the installed CLI start timeout. Leave `AGENTUP_CAPABILITY_SMOKE_SKIP_REAL` unset only on a host that has those three, such as a workstation that has run `nix-shell shell.nix` and packed the first-party capabilities.

The shared installer app is the target user-facing install, maintenance, capability-module, and update dashboard. Windows `.exe` and Ubuntu `.deb` artifacts install `AgentUp.InstallerApp` as a standalone app with a bundled release payload; Ubuntu package postinstall must not auto-launch the dashboard. macOS `.pkg` postinstall launches `AgentUp.InstallerApp` with a bundled release payload. The app can install from its bundled offline payload or, when implemented, pull the latest online payload.
...
|---|---|---|---|---|
| Windows | `agent-up-windows-<rid>.exe` plus `agent-up-windows-<rid>.msi` | Windows Service with tray login auto-start when Server is selected | application `bin` directory on PATH | Start Menu and Apps & Features |
...
- Changes to `LocalInstaller.Core` require focused tests in `LocalInstaller.Core.Tests`.
- Changes to `LocalInstaller.App` require Avalonia headless tests in `LocalInstaller.App.Tests`; Agent-Up installer flow behavior that must work on real desktop backends also requires native-display coverage in `AgentUp.Tests`.
- Native package changes require `LocalInstaller.Packaging.Tests` coverage for generated metadata/tool calls and package smoke updates when the installed contract changes.
- Package and installed-service smoke validation changes require focused tests in `LocalInstaller.Smoke.Tests`.
- Changes to thin Agent-Up product entrypoints must keep `AgentUp.InstallerApp`, `AgentUp.Packaging`, and `AgentUp.PackageSmoke` in `agent-up.sln`.
