...
Ubuntu .NET CI is five jobs after version: payloads, tests, GUI tests, runtime capability E2E, and coverage. Native package jobs wait on payloads, tests, GUI tests, runtime capability E2E, coverage, performance gates, and the test-kind watchdog plus version. The payload job is the only .NET job that uploads GitHub Actions artifacts; it publishes reusable .NET payloads for native package jobs. The test job runs every `*Tests.csproj` except `AgentUp.Tests` in deterministic path order, writes results to a directory per test project, and publishes TRX test results even when a suite fails. The GUI test job installs only the runtime WebKitGTK package needed by the native display tests, retries browser-test dependency downloads and repairs interrupted apt package state between install attempts, then runs `AgentUp.Tests` (native-display, then HeadlessE2E), excluding `RuntimeCapabilityE2E`. The Runtime capability E2E job packs first-party `dotnet` and `docker` modules, starts the repository-root `agent-up.json` from Desktop, and replays the recorded Example Web validation flow. It runs at detailed console verbosity and with `--blame-crash`, so the stage the fixture reached and a dump of a dead test host both survive into the job log and the uploaded results. The native-display `AgentUp.Tests` project may retry once on Ubuntu, but failed attempts must preserve the failing test process exit code so aborted WebView runs cannot be reported as successful.

The coverage job collects Cobertura coverage through `coverlet.runsettings` by running the non-GUI test projects itself, verifies that each directory contains a Cobertura report, and explicitly uploads those reports to Codecov with a distinct test-project flag; this preserves which test module contributed coverage to each instrumented production project. The same runsettings file sets a 30-second default NUnit test timeout and a 1-minute testhost hang abort. The GUI test job uploads `AgentUp.Tests` coverage under the `agentup-tests` flag after Test Report. Patch coverage is enforced in the coverage job, so a coverage miss cannot hide the TRX results already published by the test jobs. Native release runners download those payloads, execute the prebuilt framework-dependent `AgentUp.Tests` artifact on macOS and Windows against the matching .NET runtime those jobs install, and then run platform packaging and smoke validation. They do not restore or build product .NET projects.
...
The `linux-smoke` platform-tier verification check is locally runnable through `scripts/verify-linux-smoke.sh`. It self-publishes the five Linux product payloads and the packaging and smoke tools, packages them through the same release entry point as CI, and runs package validation plus packaged Server and CLI probes. A validated warm local run took 275 seconds, so this intentionally remains a minutes-long platform check rather than a fast check. macOS and Windows smoke remain `ciOnly` because their native jobs consume payloads cross-published by Ubuntu; local verification must report those checks as skipped with that reason rather than treating them as satisfied.
