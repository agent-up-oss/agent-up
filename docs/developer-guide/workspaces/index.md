...
`POST /api/workspaces/tutorial/cleanup` stops and removes every registered workspace. Desktop and Mobile first-launch onboarding is the built-in Demo server, not this endpoint.
...
Desktop and Mobile always list a built-in **Demo** server (`http://127.0.0.1:9`, display name `Demo`) as the first-launch path. Connecting to it switches the client onto an in-process fake backend defined by `AgentUp.FakeServer/definition.json` and shipped inside both executables. Real API clients stay unchanged: Desktop intercepts `HttpClient` with `FakeServerMessageHandler`, and Mobile intercepts `fetch`. Only while Demo is the active connection is app state faked; disconnecting and connecting to a real Server works without restart. Application WebViews cannot use that interceptor, so Desktop writes bundled HTML to a temp `file://` page and Mobile loads `html` / `srcDoc`. Settings lists an empty capability catalog because Demo does not host modules. Installed Desktop artifacts connect to `http://localhost:5000` by default.
