...
Designated performance-sensitive paths use BenchmarkDotNet benchmark projects. Initial coverage includes Server metrics shaping and Playwright export, verification diff/coverage parsing and path matching, commit policy validation, and browser input decoding. Each designation must point to at least one executable `[Benchmark]` method and a `slow` verification check selected by changes to the production path. That check lists both production and benchmark directories as inputs, so its receipt remains valid only for the exact code that was measured. Benchmark presence is therefore enforced through an actual measurement command rather than inferred from a folder name.
