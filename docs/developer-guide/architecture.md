...
The CI performance job also applies wall-clock watchdogs to the Linux Release test tiers after build: 60 seconds for combined Server/Desktop `Unit`, 75 seconds for `Provider`, and 180 seconds for cross-product `E2E`. These are suite-tier budgets, separate from NUnit's per-test timeout and testhost hang detection. Native packaging and release jobs depend on this performance job, so a failed gate cannot publish artifacts.
