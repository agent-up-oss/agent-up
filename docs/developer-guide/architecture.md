...
The CI performance job also applies wall-clock watchdogs to the Linux Release test tiers after build: 35 seconds for combined Server/Desktop `Unit`, 45 seconds for `Provider`, and 180 seconds for cross-product `E2E`. These are suite-tier budgets, separate from NUnit's per-test timeout and testhost hang detection.
