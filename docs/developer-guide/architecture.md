...
Designated performance-sensitive .NET paths use BenchmarkDotNet benchmark projects, while Mobile uses Mitata in feature-local `benchmark/` folders. The benchmark scope covers Server agent event framing, diagnostics redaction, application and process metrics, background process output, and Playwright export; Desktop agent-plan and idle-state presentation; Mobile transcript, plan, and Git-tree view projection; verification diff/coverage parsing and path matching; commit policy validation; and browser input decoding. Each designation must point to an executable benchmark and a `slow` verification check selected by changes to the production path. That check lists both production and benchmark inputs, so its receipt remains valid only for the exact code that was measured. Benchmark presence is therefore enforced through an actual measurement command rather than inferred from a folder name.
