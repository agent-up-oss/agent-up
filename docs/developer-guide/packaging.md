...
The shared installer app is the target user-facing maintenance and update flow. Windows `.exe` artifacts install `AgentUp.InstallerApp` as a standalone app with a bundled release payload; macOS `.pkg` postinstall launches `AgentUp.InstallerApp` with a bundled release payload.
