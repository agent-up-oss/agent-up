...
On Ubuntu, the default adapter installs the staged Desktop, Server, CLI, and tray payload into `/opt/agent-up`, registers `agent-up-server.service`, creates `/usr/bin/agent-up`, writes the desktop launcher with a `StartupWMClass`, registers `/etc/xdg/autostart/agent-up-tray.desktop`, and validates the installed state through systemd, tray autostart lookup, and a fresh-shell CLI lookup. Tray payload and login autostart are installed when the Server target is selected.
