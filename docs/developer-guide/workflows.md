...
The `linux-smoke` platform-tier verification check is locally runnable through `scripts/verify-linux-smoke.sh`. It self-publishes the five Linux product payloads and the packaging and smoke tools, packages them through the same release entry point as CI, and runs package validation plus packaged Server and CLI probes. This intentionally takes minutes rather than seconds. macOS and Windows smoke remain `ciOnly` because their native jobs consume payloads cross-published by Ubuntu; local verification must report those checks as skipped with that reason rather than treating them as satisfied.
