...
<DocMeta
  owner="AgentUp.Server (Applications, Ports, ApplicationProxy, DesktopApplications)"
  tests="AgentUp.Server.Tests/Features/Applications/"
  mcp="/mcp/browser hosted GUI tools"
  rest={'/api/workspaces/{id}/applications, /api/apps/tickets'}
/>

<DocSpine>
<DocBeat>Declare the process in `agent-up.json`</DocBeat>
<DocBeat>Start the workspace so ports are allocated</DocBeat>
<DocBeat>Open HTTP views or the ticketed streamed viewer</DocBeat>
</DocSpine>

<DocContract label="Route">POST /api/apps/tickets</DocContract>

<DocFacts label="Hosted GUI tools">
<DocFact label="inspect">desktop_inspect</DocFact>
<DocFact label="screenshot">desktop_screenshot</DocFact>
<DocFact label="click">desktop_click</DocFact>
<DocFact label="fill">desktop_fill</DocFact>
<DocFact label="press">desktop_press</DocFact>
</DocFacts>

These tools address an explicit workspace application and session generation. Desktop screenshots use the Server-owned framebuffer; coordinate input from a stale generation is rejected after an application restart.
...
Local application commands are an executable plus arguments. The Server launches that list directly and rejects pipes, redirects, variable expansion, command chaining, and subshells.
...
Allocated HTTP application ports stay bound on the Server host. Remote clients reach them through `POST /api/apps/tickets` and the `/apps/{workspaceId}/{port}` bootstrap.
...
<DocStep title="Reject unsafe writes">
Unsafe proxied writes require an Origin that matches the Server scheme, host, and port. Writes without an Origin are rejected. Abandoned tickets expire after 30 seconds.
</DocStep>
</DocSteps>
...
<DocSurfaces>
<DocSurface desktop>Applications whose Server DTO kind is `Desktop` use a separate application sub-tab labeled Desktop. Desktop retries session-scoped viewer-ticket requests while the application is starting or running, and hosts the returned Server viewer in a dedicated native view. This viewer is never used for ordinary HTTP ports.</DocSurface>
</DocSurfaces>
