...
{
    internal const string Component = "server";
    ...
    {
        var request = ReadProcessRequest(builder.Environment.EnvironmentName, typeof(SentryTelemetry).Assembly);
        if (!TryCreate(request, out var settings))
            return;

        builder.WebHost.UseSentry(options => Apply(options, settings));
    }

    internal static SentryTelemetryRequest ReadProcessRequest(string? hostEnvironmentName, Assembly assembly)
        => new(
            Dsn: Environment.GetEnvironmentVariable(DsnVariable),
    ...
    internal static void Apply(SentryAspNetCoreOptions options, SentryTelemetrySettings settings)
    {
        ApplyCore(options, settings);
    ...
    private static void ApplyCore(SentryOptions options, SentryTelemetrySettings settings)
    {
