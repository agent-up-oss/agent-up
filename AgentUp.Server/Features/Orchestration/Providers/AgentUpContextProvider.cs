...
public sealed class AgentUpContextProvider : IAgentUpContextProvider
{
    public string GetAgentUpContext() =>
        ...
        Before starting a new coding task, call guard_commits. If it succeeds with continueWorktreePath, run the task in that managed proposal worktree. If it fails, stop unless the user explicitly asked to continue existing changes.
