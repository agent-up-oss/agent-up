...
public sealed class ValidationFlowsController(ValidationFlowService service) : ControllerBase
{
    ...
    [HttpPut] public async Task<IActionResult> Save(string workspaceId, SaveValidationFlowRequest request, CancellationToken ct) =>
        Ok(await service.SaveAsync(workspaceId, request, ct));
