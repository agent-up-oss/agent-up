...
public sealed class LinuxDesktopFixtureAdapter : IDesktopFixtureAdapter
{
    ...
    private static bool IsDisplayReady()
    {
        try
        {
            ...
            {
                try { proc.Kill(); } catch (Exception ex) when (ex is InvalidOperationException or Win32Exception) { }
                return false;
            }
    ...
    public void Dispose()
    {
        try { _xvfb?.Kill(); } catch (Exception ex) when (ex is InvalidOperationException or Win32Exception) { }
        _xvfb?.Dispose();
    }
}
