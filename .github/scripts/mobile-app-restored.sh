...
if [ "$platform" = "ios" ] && [ -e "${required[0]}" ]; then
  if executable="$(plutil -extract CFBundleExecutable raw -o - "${required[0]}/Info.plist" 2>&1)" \
     && [ -n "$executable" ]; then
    required+=("${required[0]}/$executable")
  else
    echo "UNREADABLE: ${required[0]}/Info.plist does not say what the executable is ($executable)"
    missing=1
  fi
fi
