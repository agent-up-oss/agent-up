...
case "$platform" in
  macos)
    ...
    assert_file "$mounted_dmg/Agent-Up Installer.app/Contents/Info.plist"
    assert_file "$mounted_dmg/Agent-Up Installer.app/Contents/MacOS/AgentUpInstaller"
    assert_contains "$mounted_dmg/agent-up-server.plist" "/Applications/Agent-Up.app/Contents/Resources/server/AgentUp.Server"
    assert_contains "$mounted_dmg/agent-up-server.plist" "<key>ThrottleInterval</key>"
    assert_contains "$mounted_dmg/install.sh" "/usr/local/bin/agent-up"
