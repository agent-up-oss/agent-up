...
case "$platform" in
  ...
  nixos)
    ...
    assert_contains "$work_dir/flake.nix" "options.programs.agent-up ="
    assert_contains "$work_dir/flake.nix" "systemd.services.agent-up-server"
    ...
    assert_contains "$work_dir/flake.nix" "WorkingDirectory = \"\${package}/opt/agent-up/server\""
    assert_contains "$work_dir/flake.nix" "ASPNETCORE_CONTENTROOT = \"\${package}/opt/agent-up/server\""
    assert_contains "$work_dir/flake.nix" "DOTNET_CONTENTROOT = \"\${package}/opt/agent-up/server\""
    assert_contains "$work_dir/flake.nix" "DOTNET_BUNDLE_EXTRACT_BASE_DIR = \"/var/cache/agent-up\""
    assert_contains "$work_dir/flake.nix" "CacheDirectory = \"agent-up\""
    assert_contains "$work_dir/flake.nix" "\"ASPNETCORE_CONTENTROOT=\${package}/opt/agent-up/server\""
    assert_contains "$work_dir/flake.nix" "\"DOTNET_CONTENTROOT=\${package}/opt/agent-up/server\""
    assert_contains "$work_dir/flake.nix" "\"DOTNET_BUNDLE_EXTRACT_BASE_DIR=\${config.xdg.cacheHome}/agent-up\""
