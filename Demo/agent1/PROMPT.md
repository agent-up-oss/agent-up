...
Use Agent-Up MCP only. Attach `/mcp/orchestration` then `/mcp/browser` (and `/mcp/audit` if querying history). Start this workspace, then keep all browser activity inside the allocated local HTTP ports.
