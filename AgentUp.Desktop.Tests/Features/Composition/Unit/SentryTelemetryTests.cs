...
namespace AgentUp.Desktop.Tests.Features.Composition.Unit;
...
public sealed class SentryTelemetryTests
{
    ...
    public void TryCreate_usesPackagedProductionContractWhenDsnPresent()
    {
        var created = SentryTelemetry.TryCreate(
            Request(hostEnvironmentName: "Production", runtimeIdentifier: "osx-arm64"),
            out var settings);
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(settings.Rid, Is.EqualTo("osx-arm64"));
        });
    }
    ...
    public void Apply_setsErrorOnlyOptionsAndTagReleaseContract()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(options.DefaultTags["agentup.component"], Is.EqualTo("desktop"));
            Assert.That(options.DefaultTags["agentup.deployment"], Is.EqualTo("packaged"));
            Assert.That(options.DefaultTags["agentup.rid"], Is.EqualTo("win-x64"));
        });
    }
